...
export default function SearchBox(props) {
    ...
    return (
        <div className="searchbox" style={{ width: props.width || "100%" }} ref={searchBoxRef}>
            ...
            <div style={{ position: "absolute", background: "white", zIndex: "9999", width: "26%", borderRadius: "10px", marginTop: "10px" }}>
                {searchField && searchField?.length !== 0 ? searchField?.map((item, i) => {
                    let categorySlug = item?.categories?.edges[0]?.node?.slug

                    return (<Link href={postPathBySlugCategory(
                        item?.slug,
                        categorySlug
                    )} key={i} style={{ marginTop: "5px", padding: "5px 15px 8px" }}>
                        <div style={{ display: "flex", alignItems: "center", outline: "none" }}>
                            <img src={item?.featuredImage?.["node"]?.sourceUrl} style={{ width: "45px", height: "45px", borderRadius: "50%" }} alt="image" />
                            <h4 style={{ padding: "0px 5px", color: "black" }}>{item.title}</h4>
                        </div>
                    </Link>)
